...
> **Working Draft — MVP UI Skyline #001 已記錄，Golden Screens 尚未完成。**
...
- fallback behavior
- MVP Skyline 已確認、但仍須 Golden Screen / implementation evidence 才能 freeze 的 visual rules
...
↓
Golden UI #001 Overview / #002 Day     next
↓
06_RESULT_UI_SPEC refinement / freeze
```

Skyline 已固定 hierarchy、rhythm、semantic treatment 與 surface language；exact typography、spacing、token values、navigation anatomy 與 component API 仍不能在 Golden Screens / implementation prototype 前寫死。

目前對應的 implementation-planning change：

```text
openspec/changes/implement-mvp-result-ui-skyline/
```

該 change 只涵蓋 MVP Overview + Day visual implementation，不擴張 Today、Map、Reservations、hosted delivery、parser 或 CanonicalTrip scope。
