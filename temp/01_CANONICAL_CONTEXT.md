...
Trip Runtime 的 architecture mantra：
...
## 4.5 Semantic Interpreter 與責任分工

`Semantic Interpreter` 是 `UnifiedSourceDocument` 與 `ParsedTripDraft` 之間的穩定邊界。
...
- 讀取 source format
- 擷取並重建內容與結構訊號
- 產生 `UnifiedSourceDocument`

Semantic Interpreter 中的 LLM implementation：
...
- 在 evidence-backed Review 中修正、確認或拒絕 parser result
- 只有確認後的 `CanonicalTrip` 是產品 truth

Renderer：

- 只呈現確認後的 semantic 與 runtime priority
- 不重新理解 source，不讓 LLM 決定 UI
...
`UnifiedSourceDocument` 是 source-level normalized representation；`Semantic Interpreter` 可以是 deterministic、LLM 或 hybrid implementation；`CanonicalTrip` 才是產品層 confirmed truth。
...
目前實作的 trust boundary 另有以下不變條件：

- `CanonicalTrip` 的 schema validation 同時負責 shape 與 runtime semantics；有效 IANA timezone、HTTPS-only external link、日期關係、stable ID uniqueness、reservation / todo reference 與 relation consistency 都必須在 persistence / import / Viewer entry 前成立。
- 不合法的 persisted trip 不得進入 Viewer runtime。應回到可用的 Home recovery surface，且不得靜默刪除原始 local value。
- Review 必須逐筆顯示 parser ambiguity、conflict、low-confidence、broken evidence 與 preserved reference blocks；只顯示 warning 數量不算 surfaced。
- Traveler-added item 沒有 source evidence 時要標示為 user override；不得替它虛構 excerpt。
- Runtime derivation 掛掉時，Overview 與靜態 Day itinerary 仍必須可看；NOW / NEXT / countdown / leave-by 必須省略而不是猜測。
...
Result UI 的 final visual system 尚未 freeze，但 MVP UI Skyline #001 已於 2026-08-31 記錄。

目前採用的 MVP visual thesis：
...
第一輪只呈現 austere editorial document、缺少「要出去玩」感的方向已被淘汰。採用版以 purposeful route / place atmosphere、明亮旅行色彩、旅行手帳 cues 與真實 itinerary 語氣補回期待感，同時維持 source precision、semantic hierarchy 與 utility priority。
...
`06_RESULT_UI_SPEC.md` 目前是 working draft，用來固定 Renderer boundary、semantic grammar 與已接受的 MVP Skyline。Skyline 已固定 hierarchy、rhythm、semantic treatment 與 surface language；exact typography、spacing、token values、navigation anatomy 與 component API 仍須經 Golden Screens / implementation prototype 驗證。

目前以獨立 `implement-mvp-result-ui-skyline` change 規劃 Overview + Day 的 visual implementation。這是 presentation-only track，不修改 CanonicalTrip、parser、Review、persistence、Today runtime contract 或 External Markdown Benchmark #001 gate。
...
MVP UI Skyline #001 已完成方向確認，Overview + Day 的 Golden Screen implementation 可作為平行 presentation track 進行，但不得延後、放寬或取代上述 parser acceptance gate。Today、NOW / NEXT、Map 與 Reservations 不在這個 MVP visual change 內；既有 functional contract 仍由原 V0 / Runtime 規格管理。
...
MVP UI Skyline #001                    ✓ recorded (Overview + Day)
Golden Screens                         not completed
MVP Result UI implementation change    planned, not implemented
06 Result UI Spec                      working draft, Skyline recorded
