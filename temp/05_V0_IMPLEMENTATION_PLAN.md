...
AI output 必須經過真正的 schema validation。
...
Code 負責 deterministic validation。
...
Renderer architecture 可以現在實作，但 Result UI visual system 不應在沒有視覺探索的情況下直接 freeze。
...
`06_RESULT_UI_SPEC.md` 目前是 working draft。

因此：

- Parser / Validator / Review 不需要等待 Skyline。
- 現有 Overview / Day / Today prototype 可繼續作 functional baseline。
- 新的 final typography / spacing / surface / color / navigation rule 不應只靠 agent 自行腦補。
- Golden Screens 成立後，再把確認過的 UI grammar 與 visual system 回寫 06。
