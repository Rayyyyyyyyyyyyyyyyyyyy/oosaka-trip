...
> **取得足夠的內容與結構訊號，讓後續 semantic parser 不必直接理解檔案格式。**
...
後續 semantic parser 應盡量依賴 `UnifiedSourceDocument`，而不是直接依賴：
...
`06_RESULT_UI_SPEC.md` 定義這個 presentation / visual boundary；目前仍是 working draft，visual rules 要經 `UI Skyline → Golden Screens` 後再 freeze。
