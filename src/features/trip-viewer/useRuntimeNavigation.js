...
export function useRuntimeNavigation(canonicalTrip, days) {
  const [runtime, setRuntime] = useState(() =>
    getSafeTripRuntime(canonicalTrip),
  );
  const [view, setView] = useState(() =>
    runtime.available && runtime.phase === "during" ? "today" : "overview",
  );
  const [date, setDate] = useState(() =>
    runtime.available && runtime.phase === "during"
      ? runtime.date
      : days[0].date,
  );
  ...
  const selectView = (nextView) => {
    if (nextView === "today" && runtime.available && runtime.phase === "during")
